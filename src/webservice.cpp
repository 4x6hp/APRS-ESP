...
static String buildBatteryRows()
{
    ...
    else
    {
        ...
        case XPOWERS_AXP2101_CHG_DONE_STATE:
            ...
            break;
        default:
            state = vbusIn ? "Standby" : "Discharging";
            break;
        }
    }
    ...
    if (usbPlugged)
        rows += "<tr><td>USB</td><td align=\"right\">Plugged</td></tr>";
#endif
    return rows;
}

static String buildBME280Rows()
{
    BMEData currentReading = readBME280Data();
    String rows = "";
    float bme280temp = currentReading.temperature;
    float bme280hum = currentReading.humidity;
    float bme280press = currentReading.pressure / 100.0F;
    rows += "<tr><td>Temperature</td><td align=\"right\">" + String(bme280temp, 1) + " &deg;C</td></tr>";
    rows += "<tr><td>Humidity</td><td align=\"right\">" + String(bme280hum, 1) + " %</td></tr>";
    rows += "<tr><td>Pressure</td><td align=\"right\">" + String(bme280press, 1) + " hPa</td></tr>";
...
void setHTML(byte page)
{
    ...
    streamTemplate("/header.html", {{"auto_refresh", autoRefresh},
...
void handle_root()
{
    ...
    if (gpsOnline)
    {
        ...
        gnssRows += "<tr><td>Coords</td><td align=\"right\">" +
                    (gpsValid ? (String(gps.location.lat(), 6) + "&deg;" + (gps.location.rawLat().negative ? "S" : "N") +
                                 " / " + String(gps.location.lng(), 6) + "&deg;" + (gps.location.rawLng().negative ? "W" : "E"))
                              : "-") +
                    "</td></tr>";
    ...
    streamTemplate("/dashboard.html", {{"last_readings_time", String(strTime)},
                                       {"cpu_temp", cpuTemp},
                                       {"free_heap", freeHeap},
...
void handle_storage()
{
    ...
    if (cardType != CARD_NONE)
    {
        ...
        cardStats = "<br><b>SD Card Size:</b> " + String((double)cardSize / 1000, 1) + "GB<br>\n"
                                                                                       "<b>Total space:</b> " +
                    String(cardTotal) + "MB<br>\n"
                                        "<b>Used space:</b> " +
                    String(cardUsed) + "MB<br>\n"
                                       "<br>Listing directory: " +
                    dirname + "<br>\n";
    ...
    streamTemplate("/storage.html", {{"card_type", cardTypeStr},
...
static void streamTemplate(const char *path, const TplVars &vars)
{
    ...
    f.close();
    for (auto &kv : vars)
        content.replace("{" + kv.first + "}", kv.second);
    ws(content);
}
...
void handle_setting()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (config.gps_mode == i)
            gpsModeOpts += " selected";
        gpsModeOpts += ">" + String(gpsMode[i]) + "</option>";
    }

    setHTML(2);
    streamTemplate("/setting.html", {{"gps_lat", String(config.gps_lat, 5)},
...
// if(config.tnc) tncInit();
    ...
    for (uint16_t d = 0; d <= 5000; d += 500)
    {
        ...
        if (config.digi_delay == d)
            digiDelayOpts += " selected";
    ...
    for (uint16_t d = 0; d <= 10000; d += 1000)
    {
        ...
        if (config.tx_timeslot == d)
            timeslotOpts += " selected";
        timeslotOpts += ">" + String(d) + "</option>";
    }

    setHTML(3);
    streamTemplate("/service.html", {{"aprs_enable_checked", config.aprs ? String("checked") : String("")},
...
void handle_radio()
{
    ...
    rxAttSection =
        ...
        "<option value=\"0\" " +
        rxAttSelF + ">11dB (stock)</option>"
                    "<option value=\"1\" " +
        rxAttSelT + ">2.5dB (R22 mod)</option>"
                    "</select></div>\n"
    ...
    setHTML(7);
    streamTemplate("/radio.html", {{"tx_freq_min", String(txFreqMin)},
...
void handle_system()
{
    ...
    setHTML(4);
    streamTemplate("/system.html", {{"current_time", String(strTime)},
...
void handle_test()
{
    ...
    if (server.hasArg("sendBeacon"))
    {
        ...
        if (config.tnc && tnc2Raw.length() > 0)
    ...
    streamTemplate("/test.html", {{"default_raw", defaultRaw}});
    wsEnd();
}
...
void handle_firmware()
{
    ...
    setHTML(5);
    streamTemplate("/firmware.html", {{"board_name", String(BOARD_NAME)},
                                      {"version", String(VERSION_FULL)},
...
void handle_configuration()
{
    ...
    sprintf(strCID, "%04X%08X", (uint16_t)(chipid >> 32), (uint32_t)chipid);
    setHTML(8);
    streamTemplate("/configuration.html", {{"board_name", String(BOARD_NAME)},
                                           {"version", String(VERSION_FULL)},
                                           {"chip_id", String(strCID)}});
    wsEnd();
}
...
void handle_api_dashboard()
{
    ...
    doc["bme280_rows"] = buildBME280Rows();
...
void SerialStatusLog()
{
    ...
    if (!PMU.isBatteryConnect())
        ...
        case XPOWERS_AXP2101_CHG_DONE_STATE:
            ...
            break;
        default:
            chgState = vbusIn ? "standby" : "discharging";
            break;
        }
    batObj["chg"] = chgState;
#else
...
void handle_api_file_post()
{
    ...
    {
        server.send(400, "text/plain", "missing path or content");
        return;
    }
    String path = server.arg("path");
    if (!path.startsWith("/"))
    ...
    if (!f)
    {
        server.send(500, "text/plain", "cannot open");
        return;
    }
...
void webService()
{
    ...
    server.on("/leaflet.min.js", []()
              { serveStatic("/leaflet.min.js", "application/javascript"); });
    server.on("/leaflet.min.css", []()
              { serveStatic("/leaflet.min.css", "text/css"); });
    server.on("/vu.js", []()
              { serveStatic("/vu.js", "application/javascript"); });
    server.on("/radio.js", []()
              { serveStatic("/radio.js", "application/javascript"); });
    server.on("/jquery.min.js", []()
              { serveStatic("/jquery.min.js", "application/javascript"); });
    server.on("/highcharts.js", []()
              { serveStatic("/highcharts.js", "application/javascript"); });
    server.on("/highcharts-more.js", []()
              { serveStatic("/highcharts-more.js", "application/javascript"); });
    server.on("/firmware.js", []()
              { serveStatic("/firmware.js", "application/javascript"); });
    server.on("/configuration.js", []()
              { serveStatic("/configuration.js", "application/javascript"); });
    server.on("/dashboard.js", []()
              { serveStatic("/dashboard.js", "application/javascript"); });
    server.on("/files.js", []()
              { serveStatic("/files.js", "application/javascript"); });
    server.on("/codemirror.min.js", []()
              { serveStatic("/codemirror.min.js", "application/javascript"); });
    server.on("/codemirror.min.css", []()
              { serveStatic("/codemirror.min.css", "text/css"); });
    server.on("/cm-theme-dracula.min.css", []()
              { serveStatic("/cm-theme-dracula.min.css", "text/css"); });
    server.on("/cm-mode-js.min.js", []()
              { serveStatic("/cm-mode-js.min.js", "application/javascript"); });
    server.on("/cm-mode-css.min.js", []()
              { serveStatic("/cm-mode-css.min.js", "application/javascript"); });
    server.on("/cm-mode-xml.min.js", []()
              { serveStatic("/cm-mode-xml.min.js", "application/javascript"); });
    server.on("/cm-mode-html.min.js", []()
              { serveStatic("/cm-mode-html.min.js", "application/javascript"); });
    ...
    server.on(
        ...
        []()
        {
            ...
            else if (upload.status == UPLOAD_FILE_WRITE)
            {
                ...
                if (fsUploadCfgFile)
                    fsUploadCfgFile.write(upload.buf, upload.currentSize);
            }
            else if (upload.status == UPLOAD_FILE_END)
            {
    ...
    server.on(
        ...
        []()
        {
            ...
            else if (upload.status == UPLOAD_FILE_END)
            {
                ...
                if (Update.end(true))
                { // true to set the size to the current
                  // progress
                    delay(3);
                }
                else
